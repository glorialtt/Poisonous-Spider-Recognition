...
# Clone the tensorflow/models repository locally and package the Object Detection API:
# From tensorflow/models/research/
# python setup.py sdist
# (cd slim && python setup.py sdist)
# and pass it the following flags (run it twice: once for training data, once for test data):
...
def create_tf_example(filename, example):

    image_path = '/Users/mac/Desktop/spider/dataset/train/'+ filename + '/' + example
    labels_path = '/Users/mac/Desktop/spider/dataset/xml/' + os.path.splitext(example)[0] + '.xml'
    # Read the image
...
def main(_):
    ...
    for filename in os.listdir(FLAGS.images_dir):
        if filename == '.DS_Store':
            continue
        for file in os.listdir(FLAGS.images_dir + filename):
            if file == '.DS_Store':
                continue
            tf_example = create_tf_example(filename, file)
